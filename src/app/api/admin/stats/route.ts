...
export async function GET(request: NextRequest) {
  try {
    ...
    const premiumSubscribers = await db.user.count({
    ...
    const familyUsers = await db.user.count({ where: { subscriptionTier: 'FAMILLE' } })
    ...
    const usersByProvince = await db.user.groupBy({
      by: ['province'],
      _count: true
    })
    
    const allTasks = await db.task.findMany({ select: { status: true } })
    const taskStats = {
